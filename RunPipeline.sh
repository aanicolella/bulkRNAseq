...
read1_1=$(awk "NR==$SGE_TASK_ID" $SEEDFILE | awk '{print $2}')
read1_2=$(awk "NR==$SGE_TASK_ID" $SEEDFILE | awk '{print $3}')
read2_1=$(awk "NR==$SGE_TASK_ID" $SEEDFILE | awk '{print $4}')
read2_2=$(awk "NR==$SGE_TASK_ID" $SEEDFILE | awk '{print $5}')
...
$nextflow $pipeline --fq1 $read1_1,$read1_2 --fq2 $read2_1,$read2_2 --ref_comb $ref --outdir $outdir --isoMethod Salmon
