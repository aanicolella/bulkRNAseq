...
colData = data.frame(model=c(rep(c("EXP_HT"), X), rep(c("EXP_KO"), Y), rep(c("EXP_WT"), Z)),sample=sampleNames)
...
dev.off()
pdf(file="EXP_REGION_modelpca.pdf")
vsdata <- vst(dds, blind=FALSE)
...
dim(resMergeHT)
resMergeHT <- resMergeHT[order(resMergeHT$pvalue),]
write.csv(resMergeHT, "EXP_REGION_HTWT-DESeq2_LFCshrink.csv")
...
dim(resMergeKO)
resMergeKO <- resMergeKO[order(resMergeKO$pvalue),]
write.csv(resMergeKO, "EXP_REGION_KOWT-DESeq2_LFCshrink.csv")
...
write.csv(resMergeKOHT, "EXP_REGION_KOHTWT-DESeq2_allgenes.csv")
...
dim(resMergeKOHT)
resMergeKOHT <- resMergeKOHT[order(resMergeKOHT$pvalue),]
write.csv(resMergeKOHT, "EXP_REGION_KOHT-DESeq2_LFCshrink.csv")
